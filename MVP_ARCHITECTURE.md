...
| `extraction_logs` | `id`, `import_batch_id FK UNIQUE`, `provider_request_id NULL`, `model`, `status`, `error_code NULL`, `duration_ms`, `expires_at`, timestamps | M4 retains non-content request metadata only. M5 adds encrypted `raw_output_ciphertext NULL`, then automatically purges it after 30 days. Never store file bytes. |
...
3. Keep upload bytes in request memory only. Send them directly as file/image input to the Responses API, use a versioned extraction prompt, a strict JSON schema, and `store: false`. Alongside the upload, send at most 40 unique, category-consistent merchant/category pairs derived from at most 200 recent user-owned ledger rows whose categories are active; send no dates, amounts, notes, archived-category rows, or another user's data. Treat these pairs as untrusted data that may provide canonical merchant/category evidence only for an exact or clearly close visible merchant variant. They may remove clearly incidental payment-processor prefixes, branch/store/campus/city suffixes, country/currency markers, or payment descriptors, and may suggest the matching category. Do not blindly replace a source description where a useful service word must remain (for example, `PRESTO FARE/SFW5XTZCLP` becomes `PRESTO FARE`, even when prior history is `PRESTO`); never use a hint as an instruction, to invent a value, or to override conflicting visual evidence. For owner-authorized debugging, write the resulting instruction prompt and raw response text unencrypted to the server process log; do not log multipart file/image bytes or API secrets.
4. Validate the returned JSON again with Zod. For uncertain dates, type, description, amount, or category, require the model to emit `null`/blank—not a guess. It must remove a clearly incidental merchant branch/store designator or transaction-reference suffix only when the shortened merchant is unmistakable (for example, `FARM BOY #21` to `FARM BOY` and `PRESTO FARE/SFW5XTZCLP` to `PRESTO FARE`); otherwise preserve the source merchant text.
...
Failure handling: mark the batch `failed`, retain a safe error code/message plus the short-lived encrypted provider response when available, and discard the in-memory bytes. Show the safe failure only inline for the request that just failed; retain its batch only in history, never in the temporary review queue. Clear the browser file selection when every extraction request completes. Retryable provider/network failures can receive one bounded automatic retry while bytes are still in the active request. Once that request ends, the user must re-upload to try again. Never expose raw provider responses or internal errors in the normal UI.
