...
- Encrypted raw-output retention record, 30-day idempotent purge, redacted operational metadata logs plus unencrypted server-process logs of the OpenAI instruction prompt and raw text response, safe failed-batch/re-upload behavior, and tests.
...
2. Create a processing batch before provider work. Hold uploaded bytes and request-local payloads only in memory; release references in `finally`, clear the browser file selection when the extraction request completes, and never write source files to disk, object storage, database, or the OpenAI Files API.
3. Use the official OpenAI JavaScript SDK Responses API with direct file/image input, `store: false`, versioned prompt, strict JSON schema, and model identifier. The prompt returns actual transactions only and blanks/nulls uncertain fields; it ignores balances, totals, limits, payment summaries, and clearly identified credit-card balance repayments (which settle prior spending); it must remove a clearly incidental merchant branch/store designator or transaction-reference suffix only when the canonical merchant remains unmistakable, with concise positive/negative examples to guide that judgment; and it never creates categories. Before each call, read at most 200 recent user-owned ledger rows and pass at most 40 unique, category-consistent merchant-to-active-category pairs (merchant description/category only; no dates, amounts, notes, archived categories, or other users) as untrusted context. A hint may supply canonical merchant/category evidence only for an exact or clearly close visible merchant variant: it may remove clearly incidental payment-processor prefixes, branch/store/campus/city suffixes, country/currency markers, or payment descriptors and suggest the matching category. It must not be a blind textual replacement where a useful service word remains—for example, `PRESTO FARE/SFW5XTZCLP` becomes `PRESTO FARE` even when prior history is `PRESTO`—nor may it invent a value or override conflicting visible evidence. Log the resulting instruction prompt and raw response text unencrypted to the server process for owner-authorized debugging; do not log multipart file/image bytes or API secrets.
...
**Required tests.** Multipart authentication/type handling; provider request and unencrypted server-log contract mocked at the boundary; malformed structured output; no automatic ledger insertion; candidate/category mapping; merchant/category-hint ownership, active-category, and conflicting-history filtering; safe failed batch; retention purge idempotence; redaction of operational metadata; cross-user import/batch access; browser upload-to-review flow with mocked provider.
